...
class SelfChallengeQuizScreenState extends State<SelfChallengeQuizScreen> {
  ...
  void initState() {
    ...
    endTime = DateTime.now().millisecondsSinceEpoch +
        Duration(minutes: widget.time!).inMilliseconds;
    pageController = PageController(initialPage: selectedPageIndex);
    countdownController =
        CountdownTimerController(endTime: endTime!, onEnd: onEnd);
    totalQue = widget.queCount! > widget.queList!.length
        ? widget.queList!.length
        : widget.queCount;
    init();
  }
  ...
  void submitQuiz() async {
    ...
    await quizHistoryService.addDocument(quizHistory.toJson()).then((v) async {
      await userDBService.updateDocument({
        UserKeys.points: FieldValue.increment(POINT_PER_QUESTION * rightAnswers)
              ...
              child: Column(
                ...
                children: [
                  Row(
                    ...
                    children: [
                      Text('Questions:${selectedPageIndex + 1}/$totalQue',
              ...
              child: Container(
                ...
                child: PageView(
                  ...
                  children: List.generate(totalQue!, (index) {
                    LiveStream()
                        .emit(answerQuestionStream, widget.queList![index]);
                    return SingleChildScrollView(
                      child: Column(
                        children: [
                          QuizQuestionComponent(
                              question: widget.queList![index]),
                          16.height,
                          if (index == 0 && totalQue != 1)
                            ...
                            Row(
                              children: [
                                ...
                                16.width,
                                gradientButton(text: 'Submit', onTap: onSubmit)
                                    .expand(),
                              ],
                            )
                          else if (totalQue == 1)
