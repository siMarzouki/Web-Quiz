...
class AppSettingService extends BaseService {
  ...
  Future<void> setAppSettings() async {
    ...
    await ref.get().then((value) async {
      if (value.docs.isNotEmpty) {
        appSettingModel = await ref.get().then((value) =>
            AppSettingModel.fromJson(
                value.docs.first.data() as Map<String, dynamic>));
      } else {
  ...
  Future<void> saveAppSettings(AppSettingModel appSettingModel) async {
    ...
    await setValue(
        TERMS_AND_CONDITION_PREF, appSettingModel.termCondition.validate());
    await setValue(
        PRIVACY_POLICY_PREF, appSettingModel.privacyPolicy.validate());
